...
object ArgonautExercises {
  ...
  def writeJsonString(value: String): String = jString(value).nospaces

  /** Exercise 2
    * Encode a simple boolean  Hint: use jBool method.
    */
  def writeJsonBoolean(value: Boolean): String = jBool(value).nospaces
  ...
  /** Exercise 3
    ...
    */
  def writeJsonArray(values: List[String]): String = jArray(values.map(jString)).nospaces
  ...
  def writeAgent(agent: Agent): String =
    Json(
      "surname" -> jString(agent.surname),
      "firstNames" -> jArray(agent.firstNames.map(jString)),
      "principal" -> jBool(agent.principal)
    ).nospaces
  ...
  def writeAgent3(agent: Agent): String = {
    ...
    val optionalFields: List[(String, Json)] = agent.agentId.toList.map("agentid" := _)
  ...
  def writeProperty(property: Property): String = {
    def encodeAgent(agent: Agent): Json = Json(
      "surname" -> jString(agent.surname),
      "firstNames" -> jArray(agent.firstNames.map(jString)),
      "principal" -> jBool(agent.principal)
    )
  ...
  def writePropertyWithEncoder(property: Property): String = {
    implicit def AgentEncoder = EncodeJson[Agent](agent => Json(
      "surname" -> jString(agent.surname),
      "firstNames" -> jArray(agent.firstNames.map(jString)),
      "principal" -> jBool(agent.principal)
    ))
